...
export const DaoDetails = () => {
  ...
  const { daos, joinDao, createProposal, addMember, removeMember, updateMemberRole, isConnected, connectWallet } = useApp();
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex items-center gap-4">
               <div className="flex items-center gap-2 text-zinc-300 bg-zinc-900 px-4 py-2 rounded-lg border border-zinc-800">
                  ...
                  <span className="font-bold">{dao?.members.length}</span>
                                   ...
                                   <td className="px-6 py-4">
                                      <div className="flex items-center gap-3">
                                         ...
                                         <div className="flex items-center gap-2 text-zinc-200">
                                            ...
                                            <Copy className="h-3 w-3 text-zinc-600 cursor-pointer hover:text-zinc-400" onClick={() => {navigator.clipboard.writeText(member.address); toast.success("Copied");}} />
                                         </div>
                                      </div>
                                   </td>
                                   <td className="px-6 py-4">
                                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${member.role === 'ADMIN' ? 'bg-green-900/30 text-green-400' : 'bg-yellow-900/30 text-yellow-400'}`}>
                                         <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${member.role === 'ADMIN' ? 'bg-green-400' : 'bg-yellow-400'}`}></span>
                                         {member.role}
                                      </span>
                                   </td>
                                   <td className="px-6 py-4 text-right">
                                       <div className="relative inline-block text-left">
                                           <Button 
                                              variant="ghost" 
                                              size="icon" 
                                              onClick={(e) => { e.stopPropagation(); setActiveMenuMember(activeMenuMember === member.address ? null : member.address); }}
                                            >
                                              <MoreHorizontal className="h-4 w-4 text-zinc-500" />
                                           </Button>
                                           
                                           {activeMenuMember === member.address && (
                                               <>

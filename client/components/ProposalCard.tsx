...
export const ProposalCard: React.FC<ProposalCardProps> = ({ proposal, daoId }) => {
   ...
   const handleVote = async (type: boolean) => {
      if (voting) return;
      if (proposal.voters.includes(account?.address)) {
   ...
   const userVote = proposal.voters.includes(account?.address || '')
      ...
      >
         ...
         <div className="flex-1 flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2 text-xs mt-1">
               {(() => {
                  let d = Number(proposal.deadline);
         ...
         <div className="md:w-1/3 flex flex-col justify-center gap-4">
            <div className="text-center text-xs font-medium text-zinc-500 uppercase tracking-wider">{userVote ? "You have cast your vote" : "Cast your Vote"}</div>
            ...
            <div className="flex gap-3">
               <Button
                  ...
                  className={cn(
                     ...
                     userVote ? "bg-green-600/50 text-white hover:bg-green-700 border-transparent " : "bg-transparent border"
                  )}
               >
                  Yes
               </Button>
               <Button
                  ...
                  className={cn(
                     ...
                     userVote ? "bg-red-600/50 text-white hover:bg-red-700 border-transparent" : "bg-transparent border"
                  )}
               >
                  No
               </Button>
            ...
            <div className="flex justify-between text-xs text-zinc-500 px-1">
               <span className="text-green-400">
                  {proposal.yes} Votes ({yesPercent.toFixed(0)}%)
               </span>

               <span className="text-red-400">
                  {proposal.no} Votes ({noPercent.toFixed(0)}%)
               </span>
            </div>
